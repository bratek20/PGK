...
Program3D::Program3D(std::string vsName, std::string fsName) : ShaderProgram(vsName, fsName) {
    ...
    reverseNormalId = glGetUniformLocation(id, "ReverseNormal");
    isInstancedId = glGetUniformLocation(id, "IsInstanced");
    ...
    noiseTextureId = glGetUniformLocation(id, "NoiseTextureSampler");
}
...
void Program3D::applyInstanced(bool isInstanced){
    glUniform1i(isInstancedId, isInstanced ? 1 : 0);
}

void Program3D::applyNormalsReversed(bool hasNormalsReversed){
...
void Program3D::applyNoiseTexture(GLuint texture){
